...
async def handle_orders(message: Message, state: FSMContext):
    ...
    if get_trade_cache(user_id, 'instrument_id') is None:
        ...
        await TradeState.setting_order.set()
    ...
    elif get_trade_cache(user_id, 'limit_price') is None:
        ...
        if user_input.lower() == 'no':
            ...
            return
        data
        cache_data = trade_cache[user_id]
        if request == 'market_order':
            data = aevo.rest_create_market_order(**cache_data)
        else:
            data = aevo.rest_create_order(**cache_data)
        if 'error' in data:
            delete_trade_cache(user_id)
